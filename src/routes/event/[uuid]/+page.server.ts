...
export const actions = {
		...
		if (pollData.success) {
			const poll = (
				await db
					.insert(polls)
					.values({
						event: locals.session.event,
						title: pollData.data.title,
						createdAt: new Date()
					})
					.returning()
			).at(0);
			...
			await db.insert(pollOptions).values(
				pollData.data.options.map((value) => {
					return { poll: poll.id, value };
				})
			);
		} else {
	...
	updateVote: async ({ request, locals, params }) => {
		...
		const validationResult = updateVoteSchema.safeParse({
			pollOption: formData.get('pollOption'),
			poll: formData.get('poll')
		});

		if (!validationResult.success) {
			return fail(400, validationResult.error.message);
		}
		...
		const validOptionQuery = await db
			.select()
			.from(pollOptions)
			.innerJoin(polls, eq(pollOptions.poll, polls.id))
			.where(
				and(
		...
		if (validOptionQuery.length != 1)
			return fail(400, 'Relationship constraints were not satisified.');

		await db
			.insert(pollVotes)
			.values({ poll, pollOption, user: locals.user.id })
			.onConflictDoUpdate({
	...
	registerInterest: async ({ locals, params }) => {
		...
		const user = await db
			.update(users)
			.set({ registered: !locals.user.registered })
	...
	deleteComment: async ({ locals, params, request }) => {
		...
		const result = await db
			.delete(comments)
			.where(and(eq(comments.id, id), eq(comments.event, locals.session.event)));
	...
	deletePoll: async ({ locals, params, request }) => {
		...
		const result = await db
			.delete(polls)
			.where(and(eq(polls.id, id), eq(polls.event, locals.session.event)));
		if (result.rowsAffected < 1) {
			return fail(400, "Couldn't find specified resource.");
		}
	}
