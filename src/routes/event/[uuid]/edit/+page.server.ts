...
const daysOfWeekSchema = z
	.array(z.literal(daysOfWeek))
	.transform((val) => daysOfWeekToBitfield(new Set(val)));
...
export const actions = {
	edit: async ({ request, locals, params }) => {
		...
		if (locals.user === null || locals.session === null)
			redirect(303, `/event/${params.uuid}/login`);
		...
		if (result.success) {
			...
			return fail(400, { zodError: z.flattenError(result.error) });
		}
	},
