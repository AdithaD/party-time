...
{#snippet feed()}
	...
	{#if postMode == 'none'}
		<div class="flex justify-center space-x-4">
			...
			<button class="btn btn-neutral" onclick={() => (postMode = 'payment')} hidden
				>+ Payment</button
			>
		</div>
...
{#snippet details()}
	...
	{:then event}
		...
		{:else}
			<div class="mb-4 flex flex-col justify-between xl:flex-row">
				<h1 class="mb-4 text-4xl font-bold xl:mb-0">{event.title}</h1>
				<div class="flex gap-2">
					<button class="btn btn-accent" onclick={copyLink}>Share</button>
					<a class="btn flex-grow btn-neutral lg:flex-initial" href={`/event/${event.id}/edit`}
						>Edit</a
					>
				</div>
			</div>
					...
					<div class="collapse border border-base-300 bg-base-100">
						...
						<div class="collapse-content text-sm">
							<p class=" wrap-break-word">
								{stringIfNotEmptyOrNull(data.event.description, 'No description set.')}
							</p>
						</div>
					</div>
				</div>
...
<div
	class="relative hidden min-h-screen flex-col items-stretch justify-center space-x-8 overflow-hidden bg-gradient-to-t from-base-100 to-base-200 p-16 xl:flex xl:flex-row xl:p-0"
>
...
<div class="flex min-h-screen flex-col gap-4 bg-base-100 p-16 xl:hidden">
	<div>
		{@render details()}
	</div>
	<div>
